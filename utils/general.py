...
def arrow_direction(xywh):
    # direction of motion estimation
    global arrow
    arrow = False
    if xywh[2] < 0.95 and xywh[3] < 0.95:
        if xywh[0] + 0.5 * xywh[2] > 0.95:
            if xywh[1] + 0.5 * xywh[3] > 0.95:
                arrow = 'bottom_right'
            elif xywh[1] - 0.5 * xywh[3] < 0.05:
                arrow = 'top_right'
            else:
                arrow = 'right'
        elif xywh[0] - 0.5 * xywh[2] < 0.05:
            if xywh[1] + 0.5 * xywh[3] > 0.95:
                arrow = 'bottom_left'
            elif xywh[1] - 0.5 * xywh[3] < 0.05:
                arrow = 'top_left'
...
def plot_one_circle(img, xyxy, color=None, line_thickness=None):
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    up_left, down_right = (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3]))
    centre = (up_left[0] + down_right[0]) // 2, (up_left[1] + down_right[1]) // 2
    radius = min(centre[0] - up_left[0], centre[1] - up_left[1]) // 2
    cv2.circle(img, centre, radius, color=color, thickness=tl)


def plot_one_arrow(img, arrow, xyxy, color=None, line_thickness=None):
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    up_left, down_right = (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3]))
    centre = (up_left[0]+down_right[0])//2, (up_left[1]+down_right[1])//2
    endpoint = centre
    if arrow == 'up':
        endpoint = centre[0], (centre[1]+up_left[1])//2
    elif arrow == 'top_right':
        endpoint = (down_right[0]+centre[0])//2, (up_left[1]+centre[1])//2
    elif arrow == 'right':
        endpoint = (down_right[0]+centre[0])//2, centre[1]
    elif arrow == 'bottom_right':
        endpoint = (down_right[0]+centre[0])//2, (down_right[1]+centre[1])//2
    elif arrow == 'down':
        endpoint = centre[0], (centre[1]+down_right[1])//2
    elif arrow == 'bottom_left':
        endpoint = (up_left[0]+centre[0])//2, (down_right[1]+centre[1])//2
    elif arrow == 'left':
        endpoint = (up_left[0]+centre[0])//2, centre[1]
    elif arrow == 'top_left':
        endpoint = (up_left[0]+centre[0])//2, (up_left[1]+centre[1])//2
    # print(endpoint)
    cv2.arrowedLine(img, centre, endpoint, color=color, thickness=tl, line_type=cv2.LINE_AA, tipLength=0.5)
