...
if __name__ == '__main__':
    ...
    parser.add_argument('--number', type=int, help='merged pixels number for navigation')
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            f = 'study_%s.txt' % (Path(opt.weights).stem)  # filename to save to
            y = []
            for opt.number in range(5, 51):
                print(opt)
                length = detect()
                y.append(length)  # results and times
                numpy.savetxt(f, y, fmt='%10.4g')  # save
